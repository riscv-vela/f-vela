...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  io.vector.foreach { v =>
    when (v.wb.retire || v.wb.xcpt || (wb_ctrl.vec && wb_reg_valid)) {
      csr.io.pc := v.wb.pc
  ...
  // 수정된 지점
  // EX-stage vector legality check: re-decode the instruction in EX against the
  // current vconfig so only legal vector ops are issued to the vector unit.
  val ex_vec_valid = WireInit(Bool(), false.B)
  if (usingVector) {
    val v_decode_ex = rocketParams.vector.get.decoder(p)
    v_decode_ex.io.inst := ex_reg_inst
    v_decode_ex.io.vconfig := csr.io.vector.get.vconfig
    ex_vec_valid := v_decode_ex.io.legal
  }
  io.vector.foreach { v =>
    v.ex.valid := ex_reg_valid && (ex_ctrl.vec || rocketParams.vector.get.issueVConfig.B && ex_reg_set_vconfig) && !ctrl_killx && ex_vec_valid

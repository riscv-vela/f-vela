...
class ExecuteSequencer(supported_insns: Seq[VectorInstruction], maxPipeDepth: Int, nFUs: Int)(implicit p: Parameters) extends Sequencer[ExecuteMicroOp]()(p) {
  ...
  when (io.dis.fire) {
    ...
    val dis_ctrl = Wire(new VectorDecodedControl(supported_insns, Seq(
      ...
      PipelinedExecution, PipelineStagesMinus1, FUSel(nFUs), ReadsScalar1   // Custom: rs1(m, idx)
    ))).decode(dis_inst)
  ...
  // yookyung_generate
  io.iss.bits.use_scalar_rvs1 := inst.reads_scalar1 || inst.funct3.isOneOf(OPIVI, OPIVX, OPMVX, OPFVF) || rgather_v || rgatherei16
  io.iss.bits.scalar := Mux(inst.reads_scalar1, inst.rs1_data,
    Mux(rgather_v || rgatherei16,   // Custom: rs1(m, idx)
    rgather_eidx,

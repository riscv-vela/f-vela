...
import vela_vpu.mem._
import vela_vpu.exu._
import vela_vpu.common._
import vela_vpu.insns._
...
class VectorBackend(implicit p: Parameters) extends CoreModule()(p) with HasVectorParams {
  ...
  // @@@@ Custom (PID): dedicated issue queue + sequencer + execution unit.
  val vcissq = Module(new IssueQueue(2, 1)).suggestName("vcissq")
  val vcs = Module(new CustomSequencer(Seq(VFPID64B, VFPID32B), maxPipeDepth, 7)).suggestName("vcs")
  val vcu = Module(new CustomExecutionUnit).suggestName("vcu")

  val allSeqs = Seq(vls, vss, vps) ++ vxs.flatten ++ Seq(vcs)
  val allIssQs = Seq(vlissq, vsissq, vpissq) ++ vxissqs ++ Seq(vcissq)

  val flat_vxs = vxs.flatten
  require(flat_vxs.size == flat_vxus.size)

  // Which dispatched instruction is a PID custom op (funct3+SEW aware; authoritative split).
  val dis_is_pid = vcs.accepts(vdq.io.deq.bits)
  ...
  val issGroups = Seq(
    ...
    IssueGroup(q, seqs)
  }) ++ Seq(IssueGroup(vcissq, Seq(vcs))) // @@@@ Custom (PID) group last (keeps flat_vxu_id aligned)
  ...
  for (issq <- allIssQs) {
    ...
    // junseok_generate
    issq.io.enq.bits.pid_flag := false.B        // Custom: not a PID op by default
  }

  // @@@@ Custom: mark the PID issue queue entry so the IssueQueue reserves the full vreg span.
  vcissq.io.enq.bits.pid_flag := vdq.io.deq.bits.opcustom
  ...
  val dis_ctrl = Wire(new VectorDecodedControl(all_supported_insns, Seq(
    ...
    // yookyung_modify
    VMBitReadsVM, AlwaysReadsVM, WritesVD, WritesScalar, ScalarToVD0, ReadsScalar1  // Custom: rs1(m, idx)
  ...
  for ((group, i) <- issGroups.zipWithIndex) {
    ...
    for ((seq, j) <- group.seqs.zipWithIndex) {
      ...
      // @@@@ Custom: in-flight PID writes. Drop slot 0 (the combinational issue-cycle hazard):
      //   it would form a comb loop (older_writes -> vcs.iss -> vcu.pipe_hazards(0) -> older_writes)
      //   and is already covered by vcs.seq_hazard.wintent in the same cycle.
      val older_custom_writes = vcu.io.pipe_hazards.toSeq.drop(1).map { h =>
        Mux(h.valid, h.bits.eg_oh, 0.U)
      }.foldLeft(0.U(egsTotal.W))(_|_)

      seq.io.older_writes := older_pipe_writes | older_iter_writes | older_wintents | older_custom_writes
    ...
    // @@@@ Custom: the OPFVF fredosum/fredusum encodings are only PID ops; keep them out of
    //   every non-custom group (their FP-reduction decode entry matches funct3-agnostically).
    val rawAccepts = group.seqs.map(_.accepts(vdq.io.deq.bits))
    val accepts = if (group.seqs.contains(vcs)) rawAccepts else rawAccepts.map(_ && !dis_is_pid)
  ...
  // @@@@ Custom (PID): vs2 read + execution-unit wiring. The combinational read response
  //   aligns with iss (same convention as the ExecuteSequencer read path).
  vrf.io.custom.rvs2.req <> vcs.io.rvs2
  vcu.io.iss <> vcs.io.iss
  vcu.io.rvs2_data := vrf.io.custom.rvs2.resp
  vrf.io.custom.write <> vcu.io.write
  ...
  io.busy := vdq.io.deq.valid || allSeqs.map(_.io.busy).orR || vxus.flatten.map(_.io.busy).asUInt.orR || vcu.io.busy // @@@@ custom

...
import vela_vpu.insns.{HasVectorDecoderSignals}
...
class VectorIssueInst(implicit p: Parameters) extends CoreBundle()(p) with HasVectorParams with HasVectorDecoderSignals {
  ...
  // @@@@ Custom: PID uses the OPFVF fredosum/fredusum funct6 encodings. This tag only
  //   checks funct6, so it also fires for the real OPFVV reductions; the authoritative
  //   PID/non-PID split is done in the Backend via the CustomSequencer's funct3+SEW-aware
  //   accepts(). This tag is used purely to set pid_flag on the custom issue queue.
  def opcustom = isOpf && (funct6 === F6_PID64B || funct6 === F6_PID32B)
}
...
class BackendIssueInst(implicit p: Parameters) extends VectorIssueInst()(p) {
  ...
  // yookyung_generate
  val reads_scalar1 = Bool()  // Custom: rs1(m, idx)

  // junseok_generate
  val pid_flag = Bool()       // Custom: route to the custom (PID) issue queue / span hazards
}

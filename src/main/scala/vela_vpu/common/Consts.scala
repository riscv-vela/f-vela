...
object OPIFunct6 extends ChiselEnum {
  ...
  // yookyung_modify
  val madc, vfrope, msbc = Value  // Custom: add vfrope
  val ror, rol, _ = Value
...
trait HasVectorConsts {
  ...
  // @@@@ Custom: funct6 tags for the PID custom instructions (ported from older Saturn).
  //   These reuse the fredosum/fredusum funct6 encodings but are distinguished by
  //   funct3 == OPFVF (the real reductions are OPFVV) and by restrictSEW.
  def F6_PID64B = "b000011".U(6.W) // OPFFunct6.fredosum
  def F6_PID32B = "b000001".U(6.W) // OPFFunct6.fredusum

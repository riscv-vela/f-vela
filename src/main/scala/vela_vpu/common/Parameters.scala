...
import vela_vpu.insns.{FUSel, VFPID64B, VFPID32B} // @@@@ Custom: PID insns
...
trait HasVectorParams extends HasVectorConsts { this: HasCoreParameters =>
  ...
  val debugIdSz = 16
  def nRelease = vParams.issStructure.generate(vParams).map(_.seqs.size).reduce(_+_) + 3 // load/stores + custom(PID)

...
class VectorMemMacroOp(implicit p: Parameters) extends CoreBundle()(p) with HasVectorParams {
	...
	//dongin
	// io.mem.bits.shift := issue_inst.is_shift, io.mem.bits.cast := false.B, io.mem.bits.rs1_data := issue_inst.rs1      
  	// val shift = Bool()
  	// val cast = Bool()
...
class BackendIssueInst(implicit p: Parameters) extends VectorIssueInst()(p) {
	...
	// yookyung_generate
	val reads_scalar1 = Bool()  // Custom:  rs1(m, idx) RoPE access
	val pid_flag = Bool()       // Custom: indicates if the instruction is a PID read/write
}

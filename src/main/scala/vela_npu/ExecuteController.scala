...
class ExecuteController[T <: Data, U <: Data, V <: Data](xLen: Int, tagWidth: Int, config: GemminiArrayConfig[T, U, V])
  ...
  // Add Profiler: Create a wire to hold the ROB ID for profiling and a register to hold the completed ROB IDs for profiling
...
//Hazards
  ...
  val b_operand = Operand(b_address, b_address_rs1.is_garbage(), start_inputting_b, b_fire_counter, b_fire_started, false, 1)
  val d_operand = Operand(d_address, d_address_rs2.is_garbage(), start_inputting_d, d_fire_counter, d_fire_started, false, 2)
...
//
  ...
  }
 {
    ...
    io.im2col.resp.ready := wontolic.io.a.ready
  }
    ...
    is(waiting_for_cmd) {
      ...
      when(cmd.valid(0))
      {
        ...
        .elsewhen(DoPreloads(0) && cmd.valid(1) && (raw_hazards_are_impossible.B || !raw_hazard_pre)) {
          ...
          profile_comp_rob_ids(0) := cmd.bits(0).rob_id // Add Profiler: Store the ROB ID of the completed command for profiling
        }
        ...
        .elsewhen(DoComputes(0) && cmd.valid(1) && DoPreloads(1) && (!third_instruction_needed || (cmd.valid(2) && !raw_hazard_mulpre)))
        {
          ...
          control_state := compute
          // Add Profiler: Store the ROB ID of the completed command for profiling
...
//preload_zero일 때 padding 추가해서 pipeline 맞춰주기
  ...
  when (mesh_cntl_signals_q.io.deq.fire) {
    when (cntl.a_fire && wontolic.io.a.fire && !cntl.a_garbage && cntl.a_unpadded_cols > 0.U && !cntl.im2colling) {
  ...
  io.counter.connectEventSignal(CounterEvent.ACC_A_WAIT_CYCLE,
    !(!cntl.a_fire || wontolic.io.a.fire || !wontolic.io.a.ready) && cntl.a_read_from_acc && !cntl.im2colling)

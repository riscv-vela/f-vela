...
class VectorBackend(implicit p: Parameters) extends CoreModule()(p) with HasVectorParams {
	...
	val vcu = Module(new CustomExecutionUnit(pipeDepth = 7)).suggestName("vcu") // @@ Custom Execution Unit (PID unit) 
	...
	*/
	...
	io.busy := vdq.io.deq.valid || allSeqs.map(_.io.busy).orR || vxus.flatten.map(_.io.busy).asUInt.orR || vcu.io.busy //woojin, io.busy 상태 라인에 vcu 취합

...
import f_vela_saturn.insns.{FUSel, VFPID32B, VFPID64B} // VFPID32B, VFPID64B 추가
// import f_vela_saturn.insns.{FUSel} // VFPID32B, VFPID64B 추가
...
object VectorParams {
	...
	def refParams = minParams.copy(
		...
		tlBuffer = 8,  // dongin_modify: increase TL buffer size for better performance
...
object VXFunctionalUnitGroups {
	...
	def integerFUs(idivDoesImul: Boolean = false) = integerALUs ++ Seq(
		...
		// yookyung_generate
		RoPEUnitFactory   // Custom execution unit
	)
...
case class VectorParams(
	...
	// junseok_modify@@@@ supported_ex_insns에 VFPID64B 포함
...
trait HasVectorParams extends HasVectorConsts { this: HasCoreParameters =>
	...
	def nRelease = vParams.issStructure.generate(vParams).map(_.seqs.size).reduce(_+_) + 3 // load/stores junseok_modify@@@@ + custom execution unit

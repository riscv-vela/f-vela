// Linux userspace variant of vfpid_4f.c (custom vector PID instruction, single precision).
//
// Only difference from the bare-metal version: cycle timing uses the unprivileged
// "cycle" CSR instead of "mcycle". mcycle lives at CSR 0xB00 and is M-mode only, so
// reading it from a Linux user process (U-mode) raises an illegal-instruction trap.
// "cycle" (0xC00) is the U-mode-readable shadow of the same counter.
//

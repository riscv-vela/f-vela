// Profiler hardware test for the F-Vela Gemmini.
//
// Flow (matches the profiler usage note):
//   1. Allocate a P[] array that the accelerator will fill with profile data.
//   2. Hand its address to the accelerator with gemmini_profiler(P).
//   3. Run a Gemmini workload (a tiled matmul) so the profiler records
//      load / execute / store events into P[].
//   4. gemmini_fence() to make sure every DMA write to P[] has landed.
//   5. Decode P[] and print each event as "type, start, end" so it can be
//      visualized with profiler/profile.py.
//
// Each P[] entry is packed by the hardware as  Cat(q[2b], start_time[31b], end_cycle[31b])
// (see Profiler.scala: write_q.enq.bits := Cat(q, entry.start_time, clk_cnt)), so:
//     type  = (entry >> 62) & 0x3      // 0 = ld, 1 = ex, 2 = st
//     start = (entry >> 31) & 0x7FFFFFFF
//     end   =  entry        & 0x7FFFFFFF
//
// NOTE: the hardware also prints the same "type, start, end" lines to the console

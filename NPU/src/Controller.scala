...
class Gemmini[T <: Data : Arithmetic, U <: Data, V <: Data](val config: GemminiArrayConfig[T, U, V])
  ...
  val profiler = LazyModule(new Profiler(config, new GemminiCmd(config.reservation_station_entries)))
  ...
  when (unrolled_cmd.valid) {
    ...
    */
    ...
    .elsewhen (is_profiler_paddr){
      ...
      profiler.module.io.profiler_dram_addr := loop_cmd.bits.cmd.rs1

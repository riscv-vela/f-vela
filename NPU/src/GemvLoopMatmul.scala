...
class GemvLoopMatmulLdAReq(val block_size: Int, val coreMaxAddrBits: Int, val iterator_bitwidth: Int, val max_addr: Int, val concurrent_loops: Int) extends Bundle {
...
class GemvLoopMatmulLdA(block_size: Int, coreMaxAddrBits: Int, iterator_bitwidth: Int, max_addr: Int, input_w: Int,
                    max_block_len: Int, concurrent_loops: Int, mvin_rs2_t: MvinRs2)
                   (implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new GemvLoopMatmulLdAReq(block_size, coreMaxAddrBits, iterator_bitwidth, max_addr, concurrent_loops)))
  ...
  val dram_addr = req.dram_addr + GemvLoopMatmul.castDramOffset(dram_offset)
  ...
  when(req.dram_addr === 0.U){
    ...
    val next_i = floorAdd(i, i_blocks, req.max_i, next_k === 0.U)
...
class GemvLoopMatmulLdBReq(val block_size: Int, val coreMaxAddrBits: Int, val iterator_bitwidth: Int, val max_addr: Int, val concurrent_loops: Int) extends Bundle {
  val max_k = UInt(iterator_bitwidth.W)
  ...
  val dram_stride = UInt(coreMaxAddrBits.W)
  val transpose = Bool()
  val addr_start = UInt(log2Up(max_addr+1).W)
  val loop_id = UInt(log2Up(concurrent_loops).W)
  val is_resadd = Bool()
}

class GemvLoopMatmulLdB(block_size: Int, coreMaxAddrBits: Int, iterator_bitwidth: Int, max_addr: Int, input_w: Int,
                    max_block_len: Int, concurrent_loops: Int, mvin_rs2_t: MvinRs2, sp_banks: Int)
                   (implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new GemvLoopMatmulLdBReq(block_size, coreMaxAddrBits, iterator_bitwidth, max_addr, concurrent_loops)))
    val cmd = Decoupled(Output(new RoCCCommand))
  ...
  val dram_offset = (col_iterator * req.dram_stride + row_iterator)  * block_size.U * (input_w/8).U
  val dram_addr = req.dram_addr + GemvLoopMatmul.castDramOffset(dram_offset)

/*bank size로 wrap around*/

  // (행·열 인덱스) → scratchpad row index
  val raw_rows = (col_iterator * max_row_iterator + row_iterator) * block_size.U

  // 한 BANK 안에 들어갈 수 있는 row 수 (= entries)
  val rows_per_bank = (max_addr / sp_banks).U   // == sp_bank_entries

  val sp_offset = raw_rows % rows_per_bank      // row 번호를 bank 크기 안으로 접는다
  val sp_addr   = sp_addr_start + sp_offset     // bank 넘침 없이 주소 계산

  val blocks = Mux(row_iterator + max_blocks <= max_row_iterator, max_blocks, max_row_iterator-row_iterator)
  val cols = (blocks * block_size.U) 
  val rows = block_size.U 
  ...
  when(req.dram_addr === 0.U){
    ...
    // The order here is k, j, i
    val j_blocks = Mux(req.transpose, max_blocks, 1.U)
    val k_blocks = Mux(req.transpose, 1.U, max_blocks)

    val next_k = floorAdd(k, k_blocks, req.max_k)
    val next_j = floorAdd(j, j_blocks, req.max_j, next_k === 0.U)
...
class GemvLoopMatmulLdDReq(val block_size: Int, val coreMaxAddrBits: Int, val iterator_bitwidth: Int, val max_acc_addr: Int, val concurrent_loops: Int) extends Bundle {
  val max_j = UInt(iterator_bitwidth.W)
  val max_i = UInt(iterator_bitwidth.W)
  val pad_j = UInt(log2Up(block_size).W)
  val pad_i = UInt(log2Up(block_size).W)
...
class GemvLoopMatmulLdD(block_size: Int, coreMaxAddrBits: Int, iterator_bitwidth: Int, max_acc_addr: Int, input_w: Int,
                    acc_w: Int, max_block_len: Int, max_block_len_acc: Int, concurrent_loops: Int, mvin_rs2_t: MvinRs2)
                   (implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new GemvLoopMatmulLdDReq(block_size, coreMaxAddrBits, iterator_bitwidth, max_acc_addr, concurrent_loops)))
    val cmd = Decoupled(Output(new RoCCCommand))
  ...
  val req = Reg(new GemvLoopMatmulLdDReq(block_size, coreMaxAddrBits, iterator_bitwidth, max_acc_addr, concurrent_loops))
  ...
  when (req.dram_addr === 0.U) {
    ...
    val next_i = floorAdd(i, 1.U, req.max_i, next_j === 0.U)
...
// Compute
class GemvLoopMatmulExecuteReq(val block_size: Int, val coreMaxAddrBits: Int, val iterator_bitwidth: Int, val max_addr: Int, val max_acc_addr: Int, val concurrent_loops: Int) extends Bundle {
  val max_j = UInt(iterator_bitwidth.W)
  val max_k = UInt(iterator_bitwidth.W)
  val max_i = UInt(iterator_bitwidth.W)
  val pad_j = UInt(log2Up(block_size).W)
  ...
  val c_addr_start = UInt(log2Up(max_addr+1).W)
  val loop_id = UInt(log2Up(concurrent_loops).W)
  val skip = Bool()
  val is_mvout = Bool()
}

class GemvLoopMatmulExecute(block_size: Int, coreMaxAddrBits: Int, iterator_bitwidth: Int, max_addr: Int, max_acc_addr: Int, concurrent_loops: Int,
                        preload_rs1_t: PreloadRs, preload_rs2_t: PreloadRs,
                        compute_rs1_t: ComputeRs, compute_rs2_t: ComputeRs, max_block_len: Int, sp_banks: Int)
                       (implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new GemvLoopMatmulExecuteReq(block_size, coreMaxAddrBits, iterator_bitwidth, max_addr, max_acc_addr, concurrent_loops)))
    val cmd = Decoupled(Output(new RoCCCommand))
  ...
  val req = Reg(new GemvLoopMatmulExecuteReq(block_size, coreMaxAddrBits, iterator_bitwidth, max_addr, max_acc_addr, concurrent_loops))
...
/*bank size로 wrap around*/
  ...
  // (행·열 인덱스) → scratchpad row index
  val raw_rows = (b_col * b_max_row + b_row) * block_size.U

  // 한 BANK 안에 들어갈 수 있는 row 수 (= entries)
  val rows_per_bank = (max_addr / sp_banks).U   // == sp_bank_entries

  val sp_offset = raw_rows % rows_per_bank      // row 번호를 bank 크기 안으로 접는다
  val b_addr   = b_addr_start + sp_offset     // bank 넘침 없이 주소 계산
  
  val c_addr = c_addr_start + (i * req.max_j + j) * block_size.U

  val is_mvout = req.is_mvout
  ...
  pre_cmd_rs2.local_addr := Mux(is_mvout, cast_to_acc_addr(pre_cmd_rs2.local_addr, c_addr, accumulate = req.accumulate || k =/= 0.U, read_full = false.B)
                                , cast_to_sp_addr(pre_cmd_rs2.local_addr, c_addr))

  pre_cmd.rs1 := pre_cmd_rs1.asUInt
  pre_cmd.rs2 := pre_cmd_rs2.asUInt

  val k_loop_done = (k === (req.max_k - 1.U))
  ...
  comp_cmd_rs1._spacer2 := Cat(!k_loop_done, 0.U((comp_cmd_rs1._spacer2.getWidth-1).W)) //K loop가 끝났다는 신호 전달 -> bias buffer에서 spad로 데이터 이동
  ...
  val ldb_ahead = io.ldb_completed || io.ld_j > j || (io.ld_j === j && io.ld_kb > k)
  ...
  when(req.skip) {
    ...
    when (state === pre) {
      ...
      val next_j = floorAdd(j, 1.U, req.max_j, next_k === 0.U)
      val next_i = floorAdd(i, 1.U, req.max_i, next_j === 0.U && next_k === 0.U)
...
class GemvLoopMatmulStCReq(val block_size: Int, val coreMaxAddrBits: Int, val iterator_bitwidth: Int, val max_acc_addr: Int, val concurrent_loops: Int) extends Bundle {
  val max_k = UInt(iterator_bitwidth.W)
  val max_j = UInt(iterator_bitwidth.W)
  val max_i = UInt(iterator_bitwidth.W)
  val pad_j = UInt(log2Up(block_size).W)
...
class GemvLoopMatmulStC(block_size: Int, coreMaxAddrBits: Int, iterator_bitwidth: Int, max_acc_addr: Int, input_w: Int, acc_w: Int, max_block_len: Int, concurrent_loops: Int, mvout_rs2_t: MvoutRs2)
                   (implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new GemvLoopMatmulStCReq(block_size, coreMaxAddrBits, iterator_bitwidth, max_acc_addr, concurrent_loops)))
    val cmd = Decoupled(Output(new RoCCCommand))
  ...
  val req = Reg(new GemvLoopMatmulStCReq(block_size, coreMaxAddrBits, iterator_bitwidth, max_acc_addr, concurrent_loops))
  ...
  val ln_dram_addr = req.dram_addr + GemvLoopMatmul.castDramOffset(ln_dram_offset)
  ...
  val ex_ahead = WireInit(io.ex_completed ||
      ((req.act =/= Activation.LAYERNORM) && (req.act =/= Activation.SOFTMAX) &&
       (io.ex_j >= j + blocks || ((io.ex_j === j + blocks - 1.U) && io.ex_i > i))))
  when(req.is_resadd){
...
// Combined loop
class GemvLoopMatmulState(val iterator_bitwidth: Int, val coreMaxAddrBits: Int, val max_addr: Int, val max_acc_addr: Int) extends Bundle {
  val max_k = UInt(iterator_bitwidth.W)
  val max_j = UInt(iterator_bitwidth.W)
  val max_i = UInt(iterator_bitwidth.W)
  ...
  val a_ex_spad_id = UInt(3.W)
  val b_ex_spad_id = UInt(3.W)
  val c_ex_spad_id = UInt(3.W)
  val configured = Bool()
...
class GemvLoopMatmul(block_size: Int, coreMaxAddrBits: Int, reservation_station_size: Int, max_lds: Int, max_exs: Int, max_sts: Int,
                 sp_banks: Int,sp_bank_entries: Int , acc_banks: Int, acc_bank_entries: Int ,input_w: Int, acc_w: Int, dma_max_bytes: Int,
  ...
  val max_addr = sp_banks * sp_bank_entries
  val max_acc_addr = acc_banks * acc_bank_entries

  // Create states
  val concurrent_loops = 2
  val loops = Reg(Vec(concurrent_loops, new GemvLoopMatmulState(iterator_bitwidth, coreMaxAddrBits, max_addr, max_acc_addr)))
  ...
  val ldA = Module(new GemvLoopMatmulLdA(block_size, coreMaxAddrBits, iterator_bitwidth, max_all_addr, input_w, max_block_len, concurrent_loops, mvin_rs2_t))
  val ldB = Module(new GemvLoopMatmulLdB(block_size, coreMaxAddrBits, iterator_bitwidth, max_all_addr, input_w, max_block_len, concurrent_loops, mvin_rs2_t, sp_banks))
  val ldD = Module(new GemvLoopMatmulLdD(block_size, coreMaxAddrBits, iterator_bitwidth, max_acc_addr, input_w, acc_w, max_block_len, max_block_len_acc, concurrent_loops, mvin_rs2_t))
  val ex = Module(new GemvLoopMatmulExecute(block_size, coreMaxAddrBits, iterator_bitwidth, max_addr, max_acc_addr, concurrent_loops, preload_rs1_t, preload_rs2_t, compute_rs1_t, compute_rs2_t, max_block_len, sp_banks))
  val stC = Module(new GemvLoopMatmulStC(block_size, coreMaxAddrBits, iterator_bitwidth, max_acc_addr, input_w, acc_w, max_block_len, concurrent_loops, mvout_rs2_t))
  ...
  val is_loop_run_cmd = cmd.bits.cmd.inst.funct === GEMV_LOOP_WS
  val is_loop_config_cmd = cmd.bits.cmd.inst.funct >= GEMV_LOOP_WS_CONFIG_BOUNDS && cmd.bits.cmd.inst.funct <= GEMV_LOOP_WS_CONFIG_STRIDES_DC
  ...
  io.out.bits.from_gemv_fsm := Mux(loop_configured, true.B, cmd.bits.from_gemv_fsm)
  ...
  when(cmd.valid && is_loop_cmd && !loop_being_configured.configured) {
    ...
    switch (cmd.bits.cmd.inst.funct) {
      ...
      is (GEMV_LOOP_WS) {
        ...
        loop_being_configured.a_ex_spad_id := cmd.bits.cmd.rs1(24, 22)
        loop_being_configured.b_ex_spad_id := cmd.bits.cmd.rs1(21, 19)
        loop_being_configured.c_ex_spad_id := cmd.bits.cmd.rs1(18, 16) 
  ...
  val stall_c_a_hazard = (head_loop.c_ex_spad_id === tail_loop.a_ex_spad_id) && !is_resadd && head_loop.running && !head_loop.ex_completed
  ...
  ldA.io.req.bits.addr_start := Mux(loop_requesting_ldA.a_ex_spad_id === 0.U, loop_requesting_ldA.a_addr_start, (loop_requesting_ldA.a_ex_spad_id - 1.U) * (max_addr / sp_banks).U)
  ldA.io.req.bits.loop_id := loop_requesting_ldA_id
  ldA.io.req.bits.is_resadd := is_resadd

  ldA.io.req.valid := !loop_requesting_ldA.lda_started && loop_requesting_ldA.configured && !(loop_requesting_ldA_id === tail_loop_id && stall_c_a_hazard)
  ...
  ldB.io.req.bits.addr_start := Mux(loop_requesting_ldB.b_ex_spad_id === 0.U, loop_requesting_ldB.b_addr_start, (loop_requesting_ldB.b_ex_spad_id - 1.U) * (max_addr / sp_banks).U)
  ...
  ex.io.req.bits.is_mvout := (loop_requesting_ex.c_ex_spad_id === 0.U)

  ex.io.req.valid := !loop_requesting_ex.ex_started && loop_requesting_ex.lda_started &&
    loop_requesting_ex.ldb_started && loop_requesting_ex.ldd_started && loop_requesting_ex.configured && !(loop_requesting_ex_id === tail_loop_id && stall_c_a_hazard)

  when (ex.io.req.fire) {
    loop_requesting_ex.running := true.B
    loop_requesting_ex.ex_started := true.B

    when (loop_requesting_ex.c_dram_addr =/= 0.U && loop_requesting_ex.c_ex_spad_id === 0.U) {
  ...
  when (ldD.io.req.fire) {
    ...
    when (loop_requesting_ldD.d_dram_addr =/= 0.U) {
  ...
  stC.io.req.bits.addr_start :=  Mux(loop_requesting_st.c_ex_spad_id === 0.U, st_c_addr_start,(loop_requesting_st.c_ex_spad_id - 1.U) * (max_addr / sp_banks).U)
  stC.io.req.bits.loop_id := loop_requesting_st_id
  stC.io.req.bits.is_resadd := is_resadd

  val need_stc =  (loop_requesting_st.c_ex_spad_id === 0.U || loop_requesting_st.act =/= 0.U)

  stC.io.req.valid := !loop_requesting_st.st_started && loop_requesting_st.ex_started && loop_requesting_st.configured && need_stc

  when (!need_stc && loop_requesting_st.ex_started && !loop_requesting_st.st_started) {

    // StC 건너뛰지만, *started / *completed 둘 다 세워 준다
    loop_requesting_st.st_started   := true.B
    loop_requesting_st.st_completed := true.B
  }
  ...
  when(is_resadd){
    ...
    ldB.io.req.bits.addr_start := loop_requesting_ldB.resadd_addr_start
  ...
  when (reset.asBool) {
    loops.zipWithIndex.foreach { case (l, i) =>
      ...
      l.b_addr_start := ((i) * (max_addr / concurrent_loops) + (max_addr / concurrent_loops / 2)).U
      l.resadd_addr_start := (i * (max_acc_addr / concurrent_loops)).U
    }
  }
}

object GemvLoopMatmul {
  def apply(in: DecoupledIO[GemminiCmd], ld_completed: UInt, st_completed: UInt, ex_completed: UInt,
            ...
            sp_bank: Int, sp_bank_entries: Int, acc_bank: Int, acc_bank_entries: Int, input_w: Int, acc_w: Int, dma_max_bytes: Int,
            mvin_rs2_t: MvinRs2, preload_rs1_t: PreloadRs, preload_rs2_t: PreloadRs,
            compute_rs1_t: ComputeRs, compute_rs2_t: ComputeRs, mvout_rs2_t: MvoutRs2)
           (implicit p: Parameters): (DecoupledIO[GemminiCmd], Bool) = {
    val mod = Module(new GemvLoopMatmul(block_size, coreMaxAddrBits, rob_size, max_lds, max_exs, max_sts,
      sp_bank, sp_bank_entries, acc_bank, acc_bank_entries, input_w, acc_w, dma_max_bytes,
